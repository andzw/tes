...
import math
import heapq
from Phrase_LDA.modelconfig.config import ModelConfig
...
class PhraseMining(object):
    ...
    def _get_stopwords(self):
        ...
        if len(self.stop_words) == 0:
            # f = open("./"+ModelConfig.PATH+"/stopwords.txt",'r')
    ...
    def preprocess_input(self,input_text):
        ...
        for doc in documents:
            documents2.append(' '.join([word for word in doc.split() if word not in self._get_stopwords()]))
    ...
    def _run_phrase_mining(self, min_support, max_phrase_size, alpha, file_name):
        ...
        print("document_range: ", document_range)
    ...
    def get_frequent_phrases(self):
        ...
        for key,value in self.true_counter.most_common():
            if value > 0 and len(key.split(" ")) > 1:
